 ...
 * @author  Andreas G鰐z <cpuidle@gmx.de>
...
#    if (!$smarty->is_cached('list.tpl', get_current_user_id().'|'.$filter.$pageno)) {
    ...
    $content = $smarty->fetch('studiolist.tpl', get_current_user_id().'|'.$filter.$pageno);
...
session_set('studiolistview', 'studiolist.php');
...
smarty_display('studiolist.tpl', get_current_user_id().'|'.$WHERES);
