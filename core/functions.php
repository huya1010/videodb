 ...
 * @version $Id: functions.php,v 1.130 2013/03/16 10:10:07 andig2 Exp $
 */
...
function validate_include($path)
{
    if (preg_match('#//|:#', $path))
    {
        errorpage('Security Error',
        "videoDB has detected an insecure include path ($path). This may be an attempt at injecting foreign code. The application has been terminated.");
    }
}
 ...
 */
function array_extract($ary, $key)
{
    return array_map(create_function('&$row', 'return $row["'.$key.'"];'), $ary);
}
 ...
 */
function array_associate($ary, $key, $value = false)
{
    $res = array();
    foreach ($ary as $row)
    {
        $res[$row[$key]] = ($value) ? $row[$value] : $row;
    }
    return $res;
}
